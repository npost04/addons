--Weapons
gear.NukeSolstice = { name="Solstice", augments={'MP+60','INT+15','"Mag.Atk.Bns."+15',}}
gear.PetSolstice = { name="Solstice", augments={'Mag. Acc.+20','Pet: Damage taken -4%','"Fast Cast"+5',}}
--Skirmish gear
--Taeon--

--MAB
gear.MABTaeonChapeau = { name="Taeon Chapeau", augments={'"Mag.Atk.Bns."+20','Weapon Skill Acc.+9','Weapon skill damage +2%',}}
--FastCast
gear.FCTaeonGloves = { name="Taeon Gloves", augments={'Mag. Acc.+14 "Mag.Atk.Bns."+14','"Fast Cast"+5','STR+1 AGI+1',}}
gear.FCTaeonBoots = { name="Taeon Boots", augments={'"Mag.Atk.Bns."+19','"Fast Cast"+5','DEX+5',}}
--Acc
gear.TPTaeonChapeau ={ name="Taeon Chapeau", augments={'Accuracy+15 Attack+15','"Dual Wield"+5','STR+7 AGI+7',}}
gear.TPTaeonGloves = { name="Taeon Gloves", augments={'Accuracy+25','"Dual Wield"+5','STR+7 DEX+7',}}
gear.TPTaeonTights = { name="Taeon Tights", augments={'Accuracy+23','"Dual Wield"+5','Crit. hit damage +2%',}}
gear.TPTaeonBoots = { name="Taeon Boots", augments={'Accuracy+20 Attack+20','"Dual Wield"+4','DEX+3',}}
--Ranged

    

--Reisenjima gear

--Herculean
--accuracy herc set
gear.TAHerculeanBoots = { name="Herculean Boots", augments={'Accuracy+22','"Triple Atk."+4',}}
--Magic attack Herculean
gear.MABHerculeanBoots = { name="Herculean Boots", augments={'"Mag.Atk.Bns."+21','DEX+15','"Treasure Hunter"+1','Accuracy+20 Attack+20','Mag. Acc.+19 "Mag.Atk.Bns."+19',}}
gear.MABHerculeanTrousers = { name="Herculean Trousers", augments={'DEX+9','"Mag.Atk.Bns."+28','Accuracy+7 Attack+7','Mag. Acc.+11 "Mag.Atk.Bns."+11',}}
--refreshHerculean
gear.RefreshHerculeanTrousers = {name="Herculean Trousers", augments={'Mag. Acc.+3','"Refresh"+2','Accuracy+20 Attack+20','Mag. Acc.+2 "Mag.Atk.Bns."+2',}}
--Treasure Hunter Herculean 
gear.THHerculeanTrousers = { name="Herculean Trousers", augments={'Accuracy+1 Attack+1','Mag. Acc.+15 "Mag.Atk.Bns."+15','"Treasure Hunter"+1',}}
gear.THHerculeanBoots = {name="Herculean Boots", augments={'"Mag.Atk.Bns."+21','DEX+15','"Treasure Hunter"+1','Accuracy+20 Attack+20','Mag. Acc.+19 "Mag.Atk.Bns."+19',}}
--WSD/MAB Herculean
gear.MABWSDHerculeanGloves = { name="Herculean Gloves", augments={'Mag. Acc.+23','Weapon skill damage +4%','VIT+5','"Mag.Atk.Bns."+14',}}
gear.MABWSDHerculeanTrousers = {name="Herculean Trousers", augments={'"Mag.Atk.Bns."+24','Weapon skill damage +4%','STR+10','Mag. Acc.+12',}}
--Odyssean
--PhalanxOdyssean
gear.PhalanxOdysseanCuisses = {}
--Merlinic
--Fastcast
gear.MerlinicHoodFC = { name="Merlinic Hood", augments={'"Fast Cast"+7','Mag. Acc.+12','"Mag.Atk.Bns."+5',}}
gear.MerlinicCrackowsFC = { name="Merlinic Crackows", augments={'Mag. Acc.+16','"Fast Cast"+4','INT+10','"Mag.Atk.Bns."+2',}}
--MagicBurst
gear.MerlinicHoodMB = { name="Merlinic Hood", augments={'"Mag.Atk.Bns."+22','Magic burst dmg.+10%','INT+4','Mag. Acc.+8',}}
gear.MerlinicShalwarMB = { name="Merlinic Shalwar", augments={'Mag. Acc.+22 "Mag.Atk.Bns."+22','Magic burst dmg.+6%','VIT+15',}}
gear.MerlinicCrackowsMB = { name="Merlinic Crackows", augments={'"Mag.Atk.Bns."+25','Magic burst dmg.+7%','INT+2',}}
--
--
--
--Adhemar
--melee
gear.AdhemarWristbandsMelee = { name="Adhemar Wristbands", augments={'DEX+10','AGI+10','Accuracy+15',}}
gear.AdhemarKecksMelee = { name="Adhemar Kecks", augments={'DEX+10','AGI+10','Accuracy+15',}}
--ranged
gear.AdhemarWristbandsRanged = { name="Adhemar Wristbands", augments={'AGI+10','Rng.Acc.+15','Rng.Atk.+15',}}
--gear.AdhemarKecksRanged =
--Job Capes
--PLD capes
gear.PldEnmityCape = {name="Rudianos's Mantle", augments={'HP+60','Accuracy+20 Attack+20','Enmity+10',}}
gear.PldFCCape = {name="Rudianos's Mantle", augments={'HP+60','Eva.+20 /Mag. Eva.+20','HP+10','"Fast Cast"+10',}}

--BLU capes
gear.BluCrit = {name="Rosmerta's Cape", augments={'DEX+19','Accuracy+20 Attack+20','DEX+1','Crit.hit rate+10',}}
gear.BluTP ={ name="Rosmerta's Cape", augments={'DEX+20','Accuracy+20 Attack+20','"Store TP"+10',}}
--Cor capes

gear.CorTPBack = {name="Camulus's Mantle", augments={'DEX+20','Accuracy+20 Attack+20','Accuracy+2','"Store TP"+10',}}
gear.CorRangedWSBack = { name="Camulus's Mantle", augments={'AGI+20','Rng.Acc.+20 Rng.Atk.+20','AGI+2','Weapon skill damage +10%',}}
gear.CorMagicalWSBack = { name="Camulus's Mantle", augments={'AGI+20','Mag. Acc+20 Mag. Dmg.+20','Weapon skill damage +10%',}}
--gear.CorMeleeWSBack =